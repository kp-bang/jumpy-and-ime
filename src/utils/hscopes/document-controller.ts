...
class DocumentController {
  ...
  applyChanges(changes) {
    for (let change of changes) {
      ...
      if (
        change.range.end.line - change.range.start.line !== (change.text.match(/\r\n|\n|\r/g)?.length ?? 0) ||
        initState !== lastState
      ) {
  ...
  getScopeAt(position) {
    ...
    if (!this.grammar) return new TokensInfo(new vscode.Range(position, position), "", [])
    ...
    if (
      !this.contentChangesArray.length &&
      this.documentText[position.line] !== this.document.lineAt(position.line).text
    ) {
